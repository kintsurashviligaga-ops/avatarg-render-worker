...
const JOB_MAX_RUNTIME_MS = Math.max(
  30_000,
  Number(process.env.JOB_MAX_RUNTIME_MS || String(25 * 60 * 1000))
);

const PROGRESS_MIN_INTERVAL_MS = Math.max(
  250,
  Number(process.env.PROGRESS_MIN_INTERVAL_MS || "1500")
);
...
const WORKER_ID =
  process.env.WORKER_ID || `worker_${Math.random().toString(36).slice(2)}_${Date.now()}`;
...
function runCmd(cmd: string, args: string[], opts: any = {}): Promise<CommandResult> {
...
function isDataAudioUrl(s: string): boolean {
...
function concatFileLine(p: string): string {
  const safe = String(p).replace(/'/g, "'\\''");
  return `file '${safe}'`;
}

function buildSrt(payload: JobPayload): string {
  const editedScenes = payload?.edited?.scenes || [];
  const scenesKa = payload?.localized?.scenes_ka || [];

  const mapKa = new Map<string, SceneKa>();
  for (const s of scenesKa) {
    if (s?.id) mapKa.set(s.id, s);
  }
...
interface DbCapabilities {
  ...
  supportsWorkerId: boolean;
}

const dbCaps: DbCapabilities = {
  supportsUpdatedAt: true,
  supportsWorkerId: true,
};

async function safeUpdate(jobId: string, patch: Partial<RenderJob>): Promise<void> {
  const base = { ...patch };
  ...
  if (dbCaps.supportsWorkerId && /column .*worker_id.* does not exist/i.test(msg)) {
    ...
    const { worker_id, ...rest } = base as any;
...
async function updateJob(jobId: string, patch: Partial<RenderJob>): Promise<void> {
  await safeUpdate(jobId, patch);
}

async function getJobById(jobId: string): Promise<RenderJob | null> {
...
async function uploadFinalVideo(localPath: string, objectPath: string): Promise<string> {
...
async function claimNextJob(): Promise<RenderJob | null> {
  try {
    ...
    logInfo(`🎯 Claimed job ${(data as any).id}`);
...
async function processJob(job: RenderJob): Promise<void> {
  const jobId = job?.id;
  let payload: JobPayload | undefined = job?.payload;
  ...
  const writeProgress = async (progress: number): Promise<void> => {
    const t = Date.now();
    if (t - lastProgressWrite < PROGRESS_MIN_INTERVAL_MS) return;
    lastProgressWrite = t;
    await updateJob(jobId, { progress });
  };

  const baseStartPatch: Partial<RenderJob> = {
  ...
  try {
    ...
    for (const v of Array.isArray(videos) ? videos : []) {
      if (v?.sceneId && v?.videoUrl) videoMap.set(v.sceneId, v.videoUrl);
    }

    const audioMap = new Map<string, string>();
    for (const a of Array.isArray(voiceovers) ? voiceovers : []) {
      if (a?.sceneId && a?.audioUrl) audioMap.set(a.sceneId, a.audioUrl);
    }
    ...
    for (let i = 0; i < editedScenes.length; i++) {
      ...
      if (!srcVideoUrl) throw new Error(`Missing videoUrl for ${sceneIdRaw}`);
    ...
    if (segmentPaths.length === 0) throw new Error("No segments produced");
    ...
    if (canBurnSubs) {
      ...
      const subFilter =
        `subtitles='${srtEsc}':fontsdir='${fontDirEsc}':` +
